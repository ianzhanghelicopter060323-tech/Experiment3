...
1. 定义一个 `Counter` 类。
2. 有静态数据成员 `total`，记录 `add()` 被调用了多少次。
3. 有普通成员函数 `add()`，每调用一次使 `total++`。
4. 有静态成员函数 `showTotal()`，在没有定义对象时也能显示初始值。
5. 在 `main()` 函数中测试：
   - 先不创建对象，调用 `Counter::showTotal()` 显示初始值；
   - 创建两个 `Counter` 对象；
   - 分别通过对象调用若干次 `add()`；
   - 再调用 `Counter::showTotal()` 显示总调用次数。
